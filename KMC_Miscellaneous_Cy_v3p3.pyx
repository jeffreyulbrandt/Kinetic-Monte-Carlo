...
cdef void Calc_Surface_3D(np.int32_t[:,:,::1] Lattice, np.int32_t[:,::1] Surface):
    """ Covert 3D Lattice to 1D Surface """
    ...
    for i in range(Lattice.shape[0]):
        for j in range(Lattice.shape[1]):
            ...
            for k in range(1,Lattice.shape[2]):
                if Lattice[i,j,k] > 0:
                    Surface[i,j] += 1
...
cdef void Calc_Surface_2D(np.int32_t[:,::1] Lattice, np.int32_t[::1] Surface):
    """ Covert 2D Lattice to 1D Surface """
    cdef Py_ssize_t i,k
    ...
    for i in range(Lattice.shape[0]):
        ...
        # calculate coverage and height
        for k in range(1,Lattice.shape[1]):
            if Lattice[i,k] > 0:
                Surface[i] += 1
...
cdef class Ran:
    ...
    cdef np.uint64_t Int64_n(self, np.uint64_t n):
        """ return a 64-bit unsigned random integer in the range 0 <= x < n, not including n """
        return self.Int64()%n
