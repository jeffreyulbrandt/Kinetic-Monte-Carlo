...
# cimport KMC modules
from KMC_Miscellaneous_Cy_v3p4p1 cimport Ran
...
cdef class Solver:
    ...
    def __init__(self, np.int32_t solver_type, double[::1] rates, np.int32_t length, np.int32_t[::1] ni, double[::1] ri, double[::1] total_rate, np.int32_t[::1] max_ni, np.uint64_t seed):
    ...
    cdef double Calc_tdiff(self):
        """ calculate delta time for next diff event. """
        cdef double time_diff
        
        time_diff = -(1.0/self.total_rate[0])*log(self.Rand.Float64())
